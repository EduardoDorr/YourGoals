...
public interface IReadableRepository<TEntity> where TEntity : BaseEntity
{
    ...
    Task<PaginationResult<TEntity>> GetAllByAsync(Expression<Func<TEntity, bool>> predicate, int page = 1, int pageSize = 10, CancellationToken cancellationToken = default);
}